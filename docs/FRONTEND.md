...
(`localStorage['gb-token']`; when storage is blocked, a copy in memory, so the session lasts for
the page; when only a write fails, the copy wins until another tab stores a token or clears it).
Non-2xx → throws `ApiError { status, code, message, fields? }`.
A 401 on any request except login/register clears the session and redirects to
`/login?next=<current path>`, unless the token changed while the request was in flight (a
password change re-issues it): the request is then retried once with the current token. A 401
that arrives while `patchSession` (PATCH /auth/me, which re-issues the token) is in flight first
waits for it. 204 → `undefined`.
...
`['projects']` when project data/counts change; issue update/move/delete and column
rename/delete, project delete and leaving a project also every other project's issue details, which show linked issues). Mutation hooks do **not** do optimistic updates;
...
- `realtime/useProjectRealtime(key)`: websocket to `/api/projects/{key}/ws?token=…`, on message refreshes `['project', key]` + `['issues']` + `['activity']` + other projects' issue details (they show linked issues) (debounced ~300ms) with `refreshProject`, which never cancels a refetch already running (it refetches once more after it instead), reconnect with exponential backoff, closes on unmount. A close with 1008 (token or access revoked) refreshes the project and a socket that never opened re-checks `/auth/me`, so an ended session signs out instead of retrying silently.
...
- Backlog section: `data-testid="backlog-section-<sprintId>"` or `backlog-section-backlog`; row: `data-testid="backlog-row-<ISSUE-KEY>"` (on the row's control, the summary button that takes focus and keyboard moves; the key link and epic chip sit beside it).
